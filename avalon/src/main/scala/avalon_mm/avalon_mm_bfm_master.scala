...
import avalon_mm.avalon_mm_config.{AV_ADDRESS_W, AV_BURSTCOUNT_W, AV_DATA_W, AV_NUMSYMBOLS, AV_TRANSACTIONID_W, MAX_BURST_SIZE, USE_BURSTCOUNT, USE_WRITERESPONSE}
...
class MasterCommand_t {
    ...
    var byte_enbale = Array(MAX_BURST_SIZE, AV_NUMSYMBOLS) // byte choose
    ...
    var write_response_request = false
}
// Response descriptor: Should give proper response to both Write and Read
class MasterResponse_t {
    val request = REQ_IDLE
    val address = UInt((AV_ADDRESS_W).W)      // start address
    val burstcount = UInt((AV_BURSTCOUNT_W).W) // the burst data length
    val data = Array(MAX_BURST_SIZE, AV_DATA_W)  // data point to agent
    val byte_enbale = Array(MAX_BURST_SIZE, AV_NUMSYMBOLS-1) // byte choose
    val wait_latency = Array(MAX_BURST_SIZE, 32) // a vector has INT members
...
class avalon_mm_bfm_master_main[T <: MultiIOModule](dut :T, val avm_if: avalon_mm_bfm_master_if) extends PeekPokeTester(dut) {
    ...
    def event_all_transactions_complete(): Boolean = {get_command_issued_queue_size() == 0 && get_command_pending_queue_size() == 0}
    ...
    def get_response_byte_enable(index: Int) : Int = master_response.byte_enbale(index)
    ...
    def get_write_response_status(): response_status = {
        if(master_response.request == REQ_WRITE){
            if(USE_WRITERESPONSE == 1 && master_response.write_response_request)
    ...
    def set_command_write_response_request(request:Boolean): Unit = command_temp.write_response_request = request
